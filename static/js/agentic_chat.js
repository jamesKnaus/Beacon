...
let conversationState = {
    step: 'welcome', // General tracking of conversation progress
    investment_strategy: '',
    boroughs: [],
    neighborhoods: [],
    property_types: [],
...
async function analyzeAndRespond(userMessage) {
    ...
    if (conversationState.step === 'welcome') {
        ...
        if (detectedStrategy) {
            ...
            displayBotMessage(`Great choice! ${domainKnowledge.investmentStrategies[detectedStrategy]} Now, which NYC boroughs are you interested in exploring? You can select multiple or say "all boroughs".`);
            
            conversationState.step = 'boroughs';
            displayTextInput("E.g., Manhattan and Brooklyn");
        } else {
        ...
        displayTextInput("E.g., Condos and townhouses");
    }
        ...
        if (minBudget && maxBudget) {
            ...
            displayTextInput("E.g., Medium risk");
        } else {
        ...
        if (messageLower.includes('no') || messageLower.includes('goodbye') || messageLower.includes('bye') || messageLower.includes("that's all")) {
            ...
            displayBotMessage("Beacon focuses on four main NYC investment strategies:<br><br>" +
                "<strong>Value Investing:</strong> Finding undervalued properties with potential for appreciation in established or improving areas.<br><br>" +
